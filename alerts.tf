...
# Trigger: High Usage (> Defined Limit per App)
...
resource "coralogix_alert" "app_high_usage_block" {
  ...
  # Update name to reflect dynamic limit isn't just "30" anymore
  name        = "${each.value}_HighUsage_BlockLogs"
  description = "Usage > ${local.app_limits[each.value]}. Enables TCO Policy to BLOCK logs."
  enabled     = true
  priority    = "P1"
...
# Trigger: Total Usage < (Sum of all Limits)
...
resource "coralogix_alert" "global_low_usage_recovery" {
  ...
  #description = "Total Usage < ${local.total_system_limit}. Disables TCO Policy (Unblocks logs) for ALL apps."
